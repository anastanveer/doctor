...
@section('content')
  <div class="admin-panel">
    ...
    <form class="qb-card" method="post" action="{{ $question->exists ? route('admin.questions.update', $question) : route('admin.questions.store') }}" enctype="multipart/form-data">
        @csrf
        @if ($question->exists)
          @method('put')
        @endif
        ...
        <div class="qb-options" style="gap:14px;">
          ...
          @if (!empty(data_get($question->meta, 'image')))
            <div style="display:flex; gap:12px; align-items:center;">
              <img src="{{ data_get($question->meta, 'image') }}" alt="{{ data_get($question->meta, 'image_alt') ?? 'Question image' }}" style="width:140px; border-radius:10px; border:1px solid rgba(15,23,42,.12);" />
              <label class="qb-radio" style="gap:6px;">
          ...
          @endif
          <input name="question_image_alt" type="text" value="{{ old('question_image_alt', data_get($question->meta, 'image_alt') ?? '') }}" placeholder="Question image alt text (optional)" style="height:44px; border-radius:8px; border:1px solid var(--border); padding:0 12px;" />
          ...
          @if (!empty(data_get($question->meta, 'explanation_image')))
            <div style="display:flex; gap:12px; align-items:center;">
              <img src="{{ data_get($question->meta, 'explanation_image') }}" alt="{{ data_get($question->meta, 'explanation_image_alt') ?? 'Explanation image' }}" style="width:140px; border-radius:10px; border:1px solid rgba(15,23,42,.12);" />
              <label class="qb-radio" style="gap:6px;">
          ...
          @endif
          <input name="explanation_image_alt" type="text" value="{{ old('explanation_image_alt', data_get($question->meta, 'explanation_image_alt') ?? '') }}" placeholder="Explanation image alt text (optional)" style="height:44px; border-radius:8px; border:1px solid var(--border); padding:0 12px;" />
